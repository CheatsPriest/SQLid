...
static void print_json_as_table(const boost::json::value& json) {
	const auto& obj = json.as_object();

	bool success = obj.at("success").as_bool();
	if (!success) {
		std::cout << "Error: " << obj.at("error").as_string() << std::endl;
		return;
	}

	if (auto it = obj.find("message"); it != obj.end()) {
		std::cout << "Message: " << it->value().as_string() << std::endl;
	}

	try {
		const auto& headers = obj.at("headers").as_array();
		const auto& types = obj.at("types").as_array();


		std::cout << "| ";
		for (const auto& col : headers) {
			std::cout << std::setw(15) << col.as_string() << " | ";
		}
		std::cout << std::endl;
	...
	try {
		...
		for (const auto& row_val : data) {
			...
			for (const auto& cell : row) {
				if (cell.is_int64()) {
					std::cout << std::setw(15) << cell.as_int64() << " | ";
				}
				else if (cell.is_double()) {
					std::cout << std::setw(15) << cell.as_double() << " | ";
				}
				else if (cell.is_bool()) {
					std::cout << std::setw(15) << (cell.as_bool() ? "true" : "false") << " | ";
				}
				else if (cell.is_string()) {
					std::cout << std::setw(15) << cell.as_string() << " | ";
				}
			}
			std::cout << std::endl;
		}
