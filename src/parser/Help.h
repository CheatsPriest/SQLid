...
static constexpr std::array<const char*, 36> helpArray = {
	...
	"Valid examples: ",
	"1) '... WHERE id == 100'",
	"2) '... WHERE id >= 10 AND id <= 80 AND name < Numa AND age > 20 OR SALARY == 1.1 LIMIT 10",
	" ",
	"--- Queries ---", 
	...
	"'TEXT'	- cString (128 chars fixed)",
	"Boolean: 'BOOL'"


};

std::vector <std::vector<variant_types>> getHelp() {
	static std::vector <std::vector<variant_types>> vec;
	if (vec.empty()) {
		for (const auto& str : helpArray) {
			vec.push_back({ str });
		}
	}
	return vec;
